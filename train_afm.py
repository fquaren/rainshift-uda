...
from data.dataset_zarr import (
    ClimateSRDatasetZarr,
    JointZarrDataset,
...
def load_stats(domain_path):
    """Load normalization_stats.json for a domain, computing and caching it
    from the training Zarr if absent. Matches the Zarr dataset's stats file
    and transform logic. Runs in the main process (before workers fork)."""
...
def build_loaders(args, src_stats, tgt_stats, batch_size=None):
    ...
    if args.joint_training:
        joint = JointZarrDataset(mk(args.source_path, "train"),
                                 mk(args.target_path, "train"))   # both use src_stats
...
def train_one_epoch(model, src_ld, tgt_ld, opt, uda_comp, method, lam_eff, beta, device, epoch, total):
    model.train()
...
def run_training(args, device, lr=None, lambda_uda=None, batch_size=None, fda_beta=None, weight_decay=None, enc_w=None):
    ...
    ss = load_stats(args.source_path)
    ts = load_stats(args.target_path)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr = train_one_epoch(
            model, src_tr, tgt_tr, opt, uc, args.uda_method, lam_eff, _beta, device, epoch, args.epochs,
        )
        sched.step()
        val = evaluate(model, src_val, device, ss)
        tgt = evaluate(model, tgt_te, device, ss)  # source-normalized outputs -> source stats for inverse transform
...
def parse_args():
    ...
    p.add_argument("--val_chunks", type=int, default=175,
    ...
    p.add_argument("--shuffle_buffer_chunks", type=int, default=8,
                   help="Rolling shuffle-buffer size in chunks for the streaming "
                        "IterableDataset (train only). Larger = better mixing, "
                        "more RAM per worker.")
    p.add_argument("--subset_chunks", type=int, default=None,
                   help="Cap the number of chunks per split (smoke tests). "
                        "Replaces the old --subset_size.")
    return p.parse_args()
