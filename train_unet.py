...
def build_loaders(args, src_stats, tgt_stats, batch_size=None):
    ...
    mk = lambda p, s, st: ClimateSRDatasetNPY(
        p, s, stats=st, subset_size=args.subset_size)

    return (
        DataLoader(mk(args.source_path, "train", src_stats),
                   bs, shuffle=True, drop_last=True, **dl_kw),
        DataLoader(mk(args.source_path, "validation", src_stats),
                   bs, shuffle=False, **dl_kw),
        DataLoader(mk(args.target_path, "train", tgt_stats),
                   bs, shuffle=True, drop_last=True, **dl_kw),
        DataLoader(mk(args.target_path, "test", tgt_stats),
                   bs, shuffle=False, **dl_kw),
    )
...
def train_one_epoch(model, src_loader, tgt_loader, optimiser, criterion,
                    uda_comp, method, lambda_uda, fda_beta, device,
                    epoch, total_epochs):
    model.train()
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            ...
            if method in ("coral", "mmd"):
                ...
                uda_loss = uda_comp["loss_fn"](
                    feats_s["bottleneck"], feats_t["bottleneck"])
            elif method == "dann":
                _, feats_t = model(x_t, s_t, extract_features=True)
                uda_loss = dann_loss(uda_comp["disc"],
                                     feats_s["bottleneck"],
                                     feats_t["bottleneck"], alpha)
...
def run_training(args, device, lr=None, lambda_uda=None, batch_size=None,
                 fda_beta=None, weight_decay=None, trial=None):
    _lr = lr or args.lr
    ...
    model = DualEncoderUNet(
        dynamic_channels=9, static_channels=2, out_channels=1, base_features=64
    ).to(device)
    if args.compile:
    ...
    opt = torch.optim.AdamW(
        list(model.parameters()) + extra_params, lr=_lr, weight_decay=_wd)
    ...
    for epoch in range(1, args.epochs + 1):
        t0 = time.time()
        tr = train_one_epoch(model, src_tr, tgt_tr, opt, criterion,
                             uda_comp, args.uda_method, _lam, _beta,
    ...
    if trial is None:
        cfg = {**vars(args), "lr": _lr, "bs": _bs, "wd": _wd,
               "lambda": _lam, "beta": _beta, "best_tgt_loss": best}
...
def _phase1_objective(trial, args, device):
    ...
    try:
        return run_training(args, device, lr=lr, weight_decay=wd,
...
def run_phase1(args, device):
    ...
    storage = RDBStorage(url=f"sqlite:///{db}",
                         engine_kwargs={"connect_args": {"timeout": 60}})

    study = optuna.create_study(
        study_name=f"phase1_{tag}", storage=storage,
        direction="minimize", load_if_exists=True,
        pruner=optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=10))

    study.optimize(lambda t: _phase1_objective(t, args, device),
...
def run_phase2(args, device):
    ...
    study.optimize(lambda t: _phase2_objective(t, args, device, base_hp),
                   n_trials=args.n_trials, timeout=args.optuna_timeout)
