...
def run_training(args, device, lr=None, lambda_uda=None, batch_size=None, fda_beta=None, weight_decay=None):
    ...
    src_stats = load_stats(args.source_path)
    tgt_stats = load_stats(args.target_path)
    ...
    for epoch in tqdm(range(1, args.epochs + 1), desc=f"Training {tag}", unit="epoch"):
        ...
        tgt = evaluate(model, tgt_te, criterion, device, src_stats)  # source-normalized outputs -> source stats for inverse transform
    ...
    if args.uda_method == "none":
        ...
        combo = (
            ...
            / "best_hp"
            / f"{_tag(args.source_path, args.target_path)}__{args.uda_method}.json"
        )
